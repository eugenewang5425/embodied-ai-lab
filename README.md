...
三维照片房间从开环回放升级为**闭环导航**。v1 的五处接线错误（PF 单位 25× 偏小、扫描时序错位、运动噪声固定 5 cm、权重递推缺失、真值代为宣布到达）已全部修正并有专属测试。修正后四组 **零接触帧**、不可达 **12/12 安全拒绝**、A/B **12/12 到达**、C/D 各 **11/12 到达**（唯一失败是 1 次规划放弃）。详见[闭环对照报告](docs/64-closed-loop-navigation.md)。
