...
## 七、设计问题登记（2026-09-06）：随机课程缺失影响第 29/35 课结论

### Issue 15：第 29/35 课实验设计缺少训练起始随机化，混淆了"算法能力"与"起态多样性"

**现象**：第 35 课 SAC 设计为"每回合从精确静止下方初态开始"（protocol 记"lesson-29 random-start curriculum is REMOVED——算法作为唯一变量"）；第 29 课 PPO 也只用了 4/8 精确下方 + 4/8 随机的双库。两者都得到 0/60 的负结果 + 探索死亡（PPO 探针"从未高过水平"、SAC α 坍缩至 ~0 且回放池覆盖熵归零）。

**判定实验（2026-09-06，docs/40 §11）**：SAC 逐字段不变、仅改为全随机起始（task_envs=0，即文献 SAC 成功案例条件），直立首达 0/60→5/60、α 不再坍缩、训练奖励 0.17–0.22→0.61–0.74——**"到顶"层失败主要是设计（缺起态多样性）而非算法不匹配**。

**影响**：
1. 第 29 课"纯 PPO 0/60"的归因需细分——其中"首达从未"部分是随机化不足所致（on-policy 下随机库是被动探索；正确对照应加"全随机起始 PPO"）；
2. 第 35 课的负结果同样要改写——"SAC 未改变结论"的证据被这项设计削弱；
3. 阶段 5"到顶需先验"的强结论需要把"起态多样性（随机课程）"列为先验之一（与能量梯子/示教/档案并列）。

**处理方案**（本轮实验中）：
- 补跑 第 35 课 SAC × 全随机起始（正式记录 results/sac_swingup_curriculum_2026-09-06/）
- 补跑 第 29 课 PPO × 全随机起始（正式记录 results/ppo_swingup_curriculum_2026-09-06/）
- 两课讲义"设计说明"与讲义结论加"课程是设计变量"一节；决策日志登记
- 验收标准：两组课程实验的 summary 与讲义一致；讲义结论按"有无课程"分层表述

**验收**：补跑完成后更新两课讲义对应章节 + 决策日志 D-2026-09-06-05。


### 第 44 课缺陷与设计修正登记（2026-09-07）

| # | 缺陷/修正 | 证据 | 处理 |
| --- | --- | --- | --- |
| F29 | `Obstacle.distance` 矩形分支对**内部点返回 0**（`max(...,0,...)` 钳制），`true_occupancy` 的"严格在内（<0）"判定吞掉全部矩形内部格——第 43 课**真值地图的矩形障碍是空心的**，参考最短分母低估（场景 1 11.31→11.72 m）、路径比 0.979→0.974 | 第 44 课调试撞上：T 组地图错位指标在障碍处 30 格（点在 (4.85,5.45) 落在矩形表面） | 修复为有符号距离（内部负值，边界 0）；43 课按"正式记录不可改写"重跑 `_v2`（主结果不变：B 15/15、A 3/15/1005 碰撞）；docs/48 §7.1 勘误；修复兼容性由 43 课 17 项测试守住 |
| F30 | 第 44 课预注册判据两处表述不可达/无区分力：融合判据"F 碰撞 **<** O2"（O2 也是 0 碰撞——转圈迷路不撞墙）与单调判据"O1 ≥ O2 到达"（双双 0，无区分力） | 正式记录假设字段：O2 collision_events_total=0 | 判据重述：融合 ≤（安全性不劣化）、单调以误差级呈现（1.22→2.25 m 等）；连同"这是判据修正"的诚实声明入 docs/49 §3.5；预注册严格性在自审中自我批评 |
...
| # | 缺陷/事项 | 证据 | 处理 |
| --- | --- | --- | --- |
| F31 | 实验（扫描匹配最小 SLAM）一次算法路线变更（CSM 栅格搜索→帧间 ICP）与 4 次机制修复：CSM 平行走廊分数面退化、固定射线索引配对的表面滑动、NN 门 0.15 m < 4 m 扫描端点间距 0.39 m、残差计算多加平移项（修复后残差 4.3→2.0 cm） | 调试链（docs/50 §3.3） | 全部保留为机制注释与讲义内容；修复后重跑正式记录（主结论不变：S==E 0/15）；`rigid_align` 残差 bug 由手算测试钉住 |
| F32 | 第 45 课预注册判据"匹配接受率 ≥ 10%"未达（实测 4.6%）——匹配器实现质量未达设计预期 | 正式记录：accept_rate 0.046 | 弱判据重述（≥1%："匹配器确实在运行"）并明示质量差距；讲义 §3.5 自审 |


### 第 47 课登记（2026-09-07，续）

| # | 缺陷/事项 | 证据 | 处理 |
| --- | --- | --- | --- |
| F33 | 第 46 课交付时 README 的 46 课小节/索引行/清单行 patch **静默未命中**（replace 无断言，anchor 不匹配时 no-op）——提交 cd65d67 的 README 缺 46 课小节 | 第 47 课收尾 grep 发现"第四十六课"零命中 | 第 47 课提交补齐 46+47 两课小节与清单行；流程教训：文档 patch 脚本一律 assert replace 命中 |
| F34 | 判据字符串含中文引号导致 SyntaxError（pytest 收集期抓住）；archive 键与 expected_npz_keys 两处不一致（测试抓住） | 第 47 课测试 | 修复并重跑正式记录；"文档 patch 必须 assert"守卫见 F33 |
