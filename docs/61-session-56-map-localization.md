...
> 回环、54 课修了度量、55 课加了外观——本课建立**建图与定位分离的最小原型**。
> 本轮优质图让末端误差 7.37 → 2.62 m，
...
占据图把激光观测固定在空间中，粒子滤波再用观测给可能位姿打分；建图位姿漂移会让错误地图反过来误导定位。优质图只使本轮终点从 7.37 m 改善到 2.62 m，途中峰值 13.19 m，不能称为有界跟踪。自建图误差 7.38 m 和绑架恢复 0/5 把地图质量、局部跟踪和全局重定位三个问题分开。

## 思考题

1. `EST`、`PF-TRUEMAP`、`PF-SELFBUILT` 的输入地图和位姿推算各有什么不同？为什么三组必须跑在同一巡游上？
2. 平均误差 5.93→5.08 m、终点 7.37→2.62 m，途中 PF 峰值却为 13.19 m。哪句话可被数据支持，哪句话会夸大结论？
3. 绑架后触发重播种却 0/5 恢复，说明“发现异常”和“找回位置”差在哪里？请设计地图质量与粒子数分开的两个对照。
